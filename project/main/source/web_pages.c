/**
 * @file web_pages.c
 * @brief 工程级 Web：`GET /` 页面、SD 图库与 MJPEG 视频 HTTP API。
 */
...
static esp_err_t video_upload_post_handler(httpd_req_t *req)
{
    ...
    while (recv_total < (size_t)req->content_len) {
        ...
        bmp_stream_init(&ms, fp, boundary, WEB_CTRL_VIDEO_UPLOAD_MAX_BYTES);
        if (body_len > 0U) {
            err = bmp_stream_feed(&ms, stash + body_off, body_len);
            if (err != ESP_OK) {
                fail_why = (err == ESP_ERR_NO_MEM) ? "over_max" : "stream_feed";
                goto cleanup;
            }
...
stream_body:
    while ((!ms.found_end) && (recv_total < (size_t)req->content_len)) {
        ...
        recv_total += (size_t)rlen2;
        err = bmp_stream_feed(&ms, chunk, (size_t)rlen2);
        if (err != ESP_OK) {
            fail_why = (err == ESP_ERR_NO_MEM) ? "over_max" : "stream_feed";
            goto cleanup;
        }
