...
static esp_err_t root_get_handler(httpd_req_t *req)
{
    static const char html[] =
        ...
        "var mode=j.mode||'—';"
        "st.innerHTML=\"<span class='chip'><strong>模式</strong>\"+esc(mode)+\"</span>\""
        ...
        "}"
        "async function run(){"
        "showLog('o','执行中…',true);setBusy(true);"
        "try{"
        "var r=await fetch('/api/cmd',{method:'POST',headers:{'Content-Type':'application/json'},"
        "body:JSON.stringify({line:document.getElementById('l').value})});"
        "showLog('o',r.status+' '+await r.text(),true);"
