...
class ThemeFeature(FeatureModule):
    id = "theme"
    title = "Theme"
    subtitle = "theme/ui icons | F/P/S/C + WiFi"
    status = "ready"
    order = 70
...
class ThemePanel(QWidget):
    def __init__(self, ctx: ProjectContext, parent: QWidget | None = None) -> None:
        ...
        tip = QLabel(
            "按键/状态皮肤：导入图做成<strong>无底圆形</strong>（PNG 透明区真正透出）；"
            "或选<strong>纯色圆</strong>不配图。缺图且非纯色时，固件字母 F/P/S/C 或绘制 Wi‑Fi。"
            f"<br/>落盘：<code>theme/ui/*.bin</code>（{skin_core.UI_ICON_SIZE}×{skin_core.UI_ICON_SIZE} RGBH，"
            f"透明色键 #{skin_core.UI_ICON_CHROMA[0]:02X}"
            f"{skin_core.UI_ICON_CHROMA[1]:02X}{skin_core.UI_ICON_CHROMA[2]:02X}）；"
        ...
        grid.addWidget(QLabel("纯色填充"), 0, 3)
        ...
        for row, (bin_stem, assets_stem, default_bg) in enumerate(skin_core.UI_ICONS, start=1):
            ...
            mode = QComboBox()
            mode.addItem("透明圆图", "image")
            mode.addItem("纯色圆", "solid")
    ...
    def refresh_previews(self, cfg: dict | None = None) -> None:
        ...
        for bin_stem, assets_stem, _ in skin_core.UI_ICONS:
            ...
            self._bg_btns[assets_stem].setEnabled(mode == "solid")
            try:
                pix = skin_core.preview_ui_icon_pixels(
                    base, bin_stem, assets_stem, mode, bg
                )
                argb = skin_core.rgb565_to_qimage_argb32(
                ...
                prev.setText("")
                prev.setStyleSheet(
                    "background:#1e2430; border:1px solid #3a4258; border-radius:28px;"
                )
